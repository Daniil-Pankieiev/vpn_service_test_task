...
class LinkUpdater:
    def __init__(self, soup: BeautifulSoup, domain: str, endpoint: str) -> None:
        self._soup = soup
        self._domain = domain
...
def update_statistics(request: HttpRequest, response: Response, domain: str) -> None:
    statistics, _ = Statistics.objects.get_or_create(domain=domain, user=request.user)
